...
def _train_variable(variable, variable_type_list=variable_type_list):
    ...
    elif isinstance(variable[0], (float, np.float32, np.float64)):
        return tf.train.Feature(
...
def _write(path, features, slide_index, slide_metadata, tile_metadata, labels):
    ...
    record = {
        "features": _train_variable(tf.reshape(features, [-1]).numpy()),
...
def write_record(path, features, tile_info, labels, structured=False):
